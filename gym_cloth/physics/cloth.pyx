...
class Cloth(object):
    ...
    def __init__(self,
        ...
        self.params = params
        # adjust dx and dy so scale is [0,1]
        self.dx = dx = self.params["cloth"]["width"] * 1.0 / self.params["cloth"]["num_width_points"]
        self.dy = dy = self.params["cloth"]["height"] * 1.0 / self.params["cloth"]["num_height_points"]
        ...
        # Pinned points and color points are colored differently for ease of visualization.
        ...
        if state:
            ...
            for r in range(height):
                for c in range(width):
                    if init_type == 'drop-vertical':
                        noise = self.np_random.rand() * 0.01 - 0.005
                        pt = Point(x = 0.5 + noise,
                                   y = dx*c,
                                   z = dy*r,
                                   boundsx=bounds[0],
                                   boundsy=bounds[1],
                                   ...
                                   identity_1=c)
                        self.pts.append(pt)
                    elif init_type == 'single-corner-fold':
                        pt = Point(x = dx*c,
                                   y = dy*r,
                                   z = 0.0,
                                   boundsx=bounds[0],
                                   ...
                                   identity_0=r,
                                   identity_1=c)
                        if pin_cond(c, r, height, width):
                            pt.pinned = True
                        self.pts.append(pt)
                    elif init_type == 'bed-like' or init_type == 'easy-start':
                        noise = self.np_random.rand() * 0.01 - 0.005
                        if r == 0:
                            noise = 0
                        pt = Point(x = 0.0 + np.abs(noise),
                                   y = dx*c,
                                   z = dy*r,
                                   boundsx=bounds[0],
                                   boundsy=bounds[1],
